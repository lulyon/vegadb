...
uint ha_example::bytes2int(char *bytes, bool bigEndian) const
{
	uint result;
	if(bigEndian)
		result = bytes[0] * (1 << 24) + bytes[1] * (1 << 16) + bytes[2] * (1 << 8) + bytes[3];
	else
		result = bytes[3] * (1 << 24) + bytes[2] * (1 << 16) + bytes[1] * (1 << 8) + bytes[0];
	return result;
}
...
int ha_example::fetch_line(uchar* buf)
{
  ...
  char rec_header[8];
  uint bytes_read = my_pread(file->fd, (unsigned char *)rec_header, sizeof(rec_header), cur_pos, MYF(MY_WME));
  ...
  /* shape(record) ID */
   uint shape_id = bytes2int(rec_header, true);
   /* content length */
   uint content_len = bytes2int(rec_header + 4, true);

  uint rec_size = content_len;
  char *rec_buf = (char *)my_malloc(rec_size, MYF(MY_WME));

  bytes_read = my_pread(file->fd, (unsigned char *)rec_buf, rec_size, cur_pos + sizeof(rec_header), MYF(MY_WME));
  if (bytes_read == MY_FILE_ERROR)
     return HA_ERR_END_OF_FILE;
  if (!bytes_read)
     return HA_ERR_END_OF_FILE;
